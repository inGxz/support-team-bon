 ...
 *  Version: Image Upload + SubType/WorkflowParams + LEAVE SYSTEM
 *
 *  ไฟล์นี้คือโค้ดเดิมของคุณ + ระบบลางาน (/leave) รวมเป็นไฟล์เดียว
 *  ส่วนที่เพิ่มใหม่ทั้งหมดมีคอมเมนต์ "===== LEAVE SYSTEM ====="
 *
 *  วิธีใช้:
 *  1. เปิด Google Apps Script ของคุณ (script.google.com)
 *  2. เลือกโค้ดทั้งหมดในไฟล์เดิม (Ctrl+A) แล้วลบ
 *  3. วางโค้ดทั้งหมดในไฟล์นี้แทน (Ctrl+V)
 *  4. กด Save (Ctrl+S)
 *  5. Deploy → Manage deployments → Edit (✏️) → Version: New version → Deploy
 *  6. ลองส่งคำขอลาจากหน้า /leave — sheet "Leave" จะถูกสร้างอัตโนมัติ
 * ============================================================
 */
...
function doGet(e) {
  ...
  if (action === "leave_list") {
    return jsonResponse(handleLeaveList_());
  }
...
function doPost(e) {
  try {
    ...
    // ===== LEAVE SYSTEM =====
    if (body.type === "leave_create") return jsonResponse(handleLeaveCreate_(body));
    if (body.type === "leave_delete") return jsonResponse(handleLeaveDelete_(body.id));
