  ...
  if (loadError || !job) return (
    <main className="min-h-screen bg-gradient-to-br from-rose-50 to-gray-100 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-lg p-6 max-w-sm w-full text-center space-y-3">
        <p className="font-bold text-gray-700">No job found</p>
        <p className="text-gray-400 text-sm">{jobId ? `Job ID: ${jobId}` : "No Job ID in URL"}</p>
        {loadErrorMsg && <p className="text-left text-xs text-red-400 bg-red-50 rounded-lg p-2 break-all">{loadErrorMsg}</p>}
        <button onClick={() => window.location.reload()} className="w-full py-2.5 rounded-xl bg-red-500 text-white text-sm font-bold hover:bg-red-600 transition">Reload</button>
      </div>
    </main>
  );
  ...
  if (done) return (
    <main className="min-h-screen bg-gradient-to-br from-green-50 to-gray-100 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-lg p-6 max-w-sm w-full text-center space-y-3">
        ...
        <p className="font-bold text-gray-700 text-lg">Sent revision request</p>
        <p className="text-gray-400 text-sm">Team received your message</p>
        <div className="bg-gray-50 rounded-xl p-3 text-left text-sm space-y-1 mt-2">
          ...
          <p className="text-gray-500">Task: <span className="font-semibold text-gray-700">{job.task}</span></p>
        </div>
      </div>
    </main>
  );
    ...
    <main className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-100 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-lg overflow-hidden w-full max-w-sm">
        <div className="bg-green-500 px-5 py-4">
          <p className="text-white font-bold text-lg">Cannot Request Revision</p>
        ...
        <div className="px-5 py-6 flex flex-col items-center gap-4 text-center">
          <p className="font-bold text-gray-800 text-base">This job has been Approved</p>
          <p className="text-gray-500 text-sm">Job {job.jobId} was already Approved</p>
          <div className="bg-green-50 border border-green-200 rounded-xl p-3 w-full">
            <p className="text-green-700 text-xs leading-relaxed">Please contact team via LINE for further assistance</p>
          </div>
        </div>
      </div>
    </main>
    ...
    <main className="min-h-screen bg-gradient-to-br from-rose-50 to-gray-100 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-lg overflow-hidden w-full max-w-sm">
        <div className="bg-gradient-to-r from-red-500 to-rose-600 px-5 py-4">
          <p className="text-white font-bold text-lg">Request Revision</p>
        ...
        <div className="px-5 py-4 space-y-4">
          ...
          <div className="space-y-1.5">
            <p className="text-sm font-semibold text-red-600">Revision Details</p>
            <textarea
          ...
          <p className="text-center text-xs text-gray-400">Team will be notified immediately</p>
        </div>
      </div>
    </main>
  );
